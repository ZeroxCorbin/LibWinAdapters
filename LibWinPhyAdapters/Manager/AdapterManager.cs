...
namespace LibWinPhyAdapters.Manager
{
    public class CNotifyAddrChange
    {
        ...
        public void ThreadProc()
        {
            try
            {
                ...
                while (true)
                {
                    uint nRetVal = NotifyAddrChange(ref pnHandle, ref oOverlapped);
                    if (nRetVal == 997) // 997 == ERROR_IO_PENDING, means that it will notify us when an IP Address changes
                    {
                        ...
                        if (WaitHandle.WaitAny(m_aoEvents) == (int)EEvents.AddrChange)
                        {
    ...
    public class AdapterManager
    {
        ...
        {
            using (AdapterWrapper adapter = GetAdapter(interfaceIndex))
            {
                if (adapter != null)
                {
                    adapter.Disable();
